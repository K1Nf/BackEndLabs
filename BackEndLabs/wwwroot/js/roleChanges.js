...
document.getElementById("getRoleStory").addEventListener("click", function () {

    let roleInput = document.getElementById("roleId");
    let roleId = roleInput.value;

    const url = "../api/ref/policy/role/" + roleId + "/story";
    fetch(url, {
        method: "GET",
        ...
        .then(data => {
            ...
            const tableBody = document.getElementById('roleTBody');
...
function displayTable(data, tableBody) {
    ...
    for (let change of data) {
        ...
        buttonManage.addEventListener("click", async function () {
            ...
            const userUrl = "../../api/ref/changes/roles/" + changeId;// + "/rollback";
